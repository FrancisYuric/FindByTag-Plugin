    ...
    private void writeFindViews() {
        ...
        for (ResIdBean resIdBean : resIdBeans) {
            ...
            if (psiClass.findFieldByName(resIdBean.getId(), false) == null) {
                String annotation = String.format("@com.app.annotation.BindByTag(\"%s\")\n", resIdBean.getTag());
                PsiAnnotation annotationClass = psiElementFactory.createAnnotationFromText(annotation, psiClass);
                String field =
                        annotation +
                                resIdBean.getCanonicalName() + " " + resIdBean.getTag() + ";";
                PsiField fieldElement = psiElementFactory.createFieldFromText(field, psiClass);
//                JavaCodeStyleManager.getInstance(psiFile.getProject()).addImport(psiFile,getClassByName(psiFile, resIdBean.getCanonicalName()));
//                PsiImportHolder psiImportHolder = psiElementFactory.createImportStatement("import ")
//                fieldElement.add(annotationClass);

//                annotationClass.add(fieldElement);
                JavaCodeStyleManager.getInstance(psiFile.getProject()).shortenClassReferences(fieldElement);
                psiClass.add(fieldElement);
//                psiClass.addBefore(fieldElement,annotationClass);
//                JavaCodeStyleManager.getInstance(psiFile.getProject()).shortenClassReferences(annotationClass);
            }
